#!/usr/bin/env python3
"""
Process DOL LCA FY2024 xlsx → aggregate employer stats → upsert to h1b_employers.
...
# Candidate column name fragments (case-insensitive) for auto-detection
...
def load_and_aggregate(xlsx_path: Path) -> pd.DataFrame:
    ...
    print(f"  Detected columns → employer={emp_col}, status={status_col}, "
    ...
    # H1B count (all filings per employer)
    ...
    # Top job titles (first 5 unique per employer)
    if title_col:
        job_titles = (
...
def run() -> None:
    print("=" * 60)
    print("MCT PathAI — Process H1B LCA Data → Supabase")
    print("=" * 60)

    if not XLSX_PATH.exists():
        print(f"ERROR: {XLSX_PATH} not found. Run download_h1b_data.py first.")
        sys.exit(1)
